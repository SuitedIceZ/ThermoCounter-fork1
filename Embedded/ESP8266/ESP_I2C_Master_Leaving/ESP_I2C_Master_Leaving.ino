...
void loop() {
 ...
 while(Wire.available()){
    ...
    if(c == '2'){ //ask minus if possible
      String GET_output = GET_request("minus",ROOM_NUMBER);
      if(GET_output == "true"){
        Serial.println("Room " + String(ROOM_NUMBER) + " minus complete");
        ...
        Wire.write("4");
        Wire.endTransmission();
        Serial.println("Room " + String(ROOM_NUMBER) + " minus fail");
      }
      else{
        Serial.println("GET Request [minus] with room " + String(ROOM_NUMBER) + " error : " + GET_output);
      }
